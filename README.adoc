...
`name`. You also have `findByMenteesName`, which traverses the `MENTORS` relationship in reverse:
given a mentee's name, it returns all `Person` nodes that have that person in their
`mentees` set.
...
reverse lookup that finds everyone who mentors a given person.
