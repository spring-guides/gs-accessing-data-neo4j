...
`TEAMMATE` relationship points from this person to each teammate in the set.
...
again. Because the relationship direction is `OUTGOING`, only Greg's outgoing `TEAMMATE`
...
and add Craig as his teammate, then persist Roy.

Craig has no outgoing `TEAMMATE` relationships defined in this example, so no additional
code is needed for Craig. You can see the results as you iterate over each team member
